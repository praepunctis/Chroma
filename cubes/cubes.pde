// original code by samuel lapointe (@samuellapointe) - "for sharing (and forking) purposes"
// > edited by kelly fesler (@praepunctis)

import ddf.minim.*;
import ddf.minim.analysis.*;
 
// use Minim library to encapsulate audio RTA
Minim minim;
AudioPlayer song;
FFT fft;
...
// > tweak these #'s to assign weights to different frequency ranges
float specLow = 0.03; // 3% --> 0.03
float specMid = 0.1;  // 12.5% --> 0.125
float specHi = 0.3;   // 20% --> 0.20
...
// > note that the entire spectrum isn't 100% signed off

// Valeurs de score pour chaque zone
// > values to track fading of different effects per frequency band
float scoreLow = 0;
float scoreMid = 0;
float scoreHi = 0;

// Valeur précédentes, pour adoucir la reduction
// > track previous values of these vars
...
// > cubes to draw on the scene
int nbCubes;
Cube[] cubes;

//Lignes qui apparaissent sur les cotés
// > lines to draw on the scene
int nbMurs = 500;
Mur[] murs;
...
void draw()
{
  ...
  // > go to the next "frame" as the song advances
